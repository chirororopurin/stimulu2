...
input_video = "B_1.mp4"    # 入力動画
temp_video = "temp_video.mp4"
temp_audio = "temp_audio.wav"
final_video = "output_with_volume_distanceB.mp4"

# --- 音声読み込み ---
base_sound = AudioSegment.from_file(sound_file)
# 元音声を少し下げる（クリップ防止）
base_sound = base_sound - 12  # 必要に応じて変更
...
# --- 距離判定パラメータ（手ごとに設定） ---
distance_thresholds = {"RIGHT": 0.08, "LEFT": 0.08}  # 音量アップ開始距離
max_distances = {"RIGHT": 0.10, "LEFT": 0.11}     # 音量最大距離
max_boost_dbs = {"RIGHT": 100, "LEFT": 100}           # 最大増幅(dB)
boost_duration_ms = 50                            # フェード時間(ms)
...
while cap.isOpened():
    ...
    if results.pose_landmarks:
        for side in ["RIGHT", "LEFT"]:
            ...
            # 手ごとの基準値で音量計算
            threshold = distance_thresholds[side]
            max_dist = max_distances[side]
            max_boost_db = max_boost_dbs[side]
            if dist > threshold:
                vol_ratio = min((dist - threshold) / (max_dist - threshold), 1.0)
                boost_db = vol_ratio * max_boost_db
                frame_boost_db = max(frame_boost_db, boost_db)
...
cap.release()
audio_track = base_sound[:video_duration_ms]
